...
export async function registerUserRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/messages/:id', async (request) => {
    ...
    const row = get<any>('SELECT * FROM messages WHERE id = ? AND to_id = ?', [id, user.id]);
  ...
  app.delete('/api/messages/:id', async (request) => {
    const user = requireUser(request);
    const id = parseId((request.params as any).id);
    run('DELETE FROM messages WHERE id = ? AND to_id = ?', [id, user.id]);
    return { ok: true };
  });
  ...
  app.post('/api/messages', async (request) => {
    ...
    const target = findUser(String(body.to ?? ''));
    if (target.id === user.id) throw badRequest('不能给自己发私信');
    const content = String(body.content ?? '').trim();
    if (!content) throw badRequest('内容不能为空');
    if (content.length > 2000) throw badRequest('内容过长');
    const info = run(
      `INSERT INTO messages (from_id, to_id, title, content, type) VALUES (?, ?, ?, ?, 'user')`,
      [user.id, target.id, String(body.title ?? '私信').slice(0, 100), content],
    );
